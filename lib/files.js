...
module.exports = {
          ...
          if (err) {
            ...
            files.forEach((file, _index) => {
              ...
              if (_index == 0) {
                ...
                fs.renameSync(
                  fullFilePath,
                  `${operationDir(rootPath)}${item.substring(
                    0,
                    4
                  )}\\${prefix}_${intToFullStringIndexSuffix(suffixInt)}.${fileExtension}`
                )
                suffixInt += 1
              } else {
                fs.renameSync(
  ...
  readFilesHashCache: (rootPath) =>{ return new Promise(function(resolve, reject) {
    try {
      let resultHash = {};
