  ...
  process: () => {
    ...
    inquirer.prompt(questions).then((answers) => {
      ...
      if (answers["confirmed"]) {
        ...
        if (fs.existsSync(`${rootPath}\\${utils.hashFileName}`)) {
          log(chalk.green("开始执行"));
          files
            .readFilesHashCache(rootPath)
            .then((allHash) => {
              glob(`${utils.operationDir(rootPath)}\\**`, (err, allFiles) => {
                allutils.forEach((file) => {
                  // let filePath = `${rootPath}\\${file}`
                  // console.log(filePath)
                  if (fs.lstatSync(file).isFile()) {
                    // log(utils.getOriginalPath(file,allHash))
                    utils.moveBackToOriginalPath(file, allHash);
                  }
                });
              });
            })
