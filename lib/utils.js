...
function intToFullStringIndexSuffix(num, fullLength = 4) {
  let result = num.toString()
  let left = fullLength - num.toString().length
  while (left > 0) {
    result = "0" + result
    left -= 1
  }
...
module.exports = {
      ...
      fs.readdir(_path, (err, files) => {
        if (err) {
          ...
          for (let i = 0; i < utils.length; i++) {
            let file = files[utils.length - i - 1]
            _index = i
            let fullFilePath = `${_path}\\${file}`
            if (_index == 0) {
              prefix = file.split("-").splice(0, 2).join("")
  ...
  twoPathFilesCountEqual: (dirPath1, dirPath2) =>{
    return new Promise( (resolve, reject)=> {
      let files1,files2;  
      fs.readdir(dirPath1, (err, files) => {
        files1 = files;
        fs.readdir(dirPath2,(err2,files) => {
          files2 = files;
          console.log(`${dirPath1} 文件数量：${files1.length}`);
          console.log(`${dirPath2} 文件数量：${files2.length}`);
          resolve(files1.length == files2.length);
        })
      })
    })
  }
