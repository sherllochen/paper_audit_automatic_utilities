...
module.exports = {
  process: () => {
    ...
    // const rootPath = "D:\\南阳报纸审核目录\\扫描\\华侨商报1960年1月";

    log(chalk.yellow(`当前操作文件夹是 ${rootPath}`))
    utils.twoPathFilesCountEqual(`${rootPath}\\01`, `${rootPath}\\02`).then(isEqual=>{
      if(!isEqual){
        log(chalk.red('01，02文件夹下文件数量不一致，请核实。'));
        utils.waitAndEnterToQuit();
      } else {
        let questions = [
          {
            type: "confirm",
          ...
          if (answers["confirmed"]) {
            ...
            inquirer.prompt(checkQuestion).then((_answers) => {
              ...
              if (_answers["checkConfirmed"]) {
                ...
                utils.cacheFilesHash(rootPath).then(() => {
                  // 创建目录
                  log(chalk.green("创建目录"))
                  utils.createTempOperationDir(rootPath)
  
                  // 移动文件
                  log(chalk.green("移动文件"))
                  utils.moveFilesToTempDir(rootPath).then(() => {
